...
import { useAdjustAffiliateBalance } from '@/hooks/apis/useAdminUsers'

interface ModalAffiliateAdjustProps {
  open: boolean
  onClose: () => void
  userData?: any
}
...
export default function ModalAffiliateAdjust({ open, onClose, userData }: ModalAffiliateAdjustProps) {
  const [type, setType] = useState<'add' | 'subtract'>('add')
  const [amount, setAmount] = useState('')
  const [description, setDescription] = useState('')

  const adjustMutation = useAdjustAffiliateBalance()
  ...
  const currentBalance = Number(userData?.affiliate_balance ?? 0)
  ...
  const handleSubmit = () => {
    ...
    adjustMutation.mutate(
      ...
      {
        onSuccess: (data) => {
          const newBalance = data?.new_affiliate_balance

          toast.success(
            `${type === 'add' ? 'Cộng' : 'Trừ'} ${new Intl.NumberFormat('vi-VN').format(numAmount)}đ số dư affiliate thành công. Số dư mới: ${new Intl.NumberFormat('vi-VN').format(newBalance ?? 0)}đ`
          )
          onClose()
        },
        onError: (error: any) => {
    ...
    >
      ...
      <DialogContent>
        <div style={{ marginBottom: 16, padding: 12, backgroundColor: '#f5f5f5', borderRadius: 8 }}>
          ...
          <Typography variant='body2' color='text.secondary'>
            Số dư affiliate hiện tại: <strong style={{ color: '#0d9488' }}>{formatVND(currentBalance)}</strong>
          </Typography>
          {amount && Number(amount) > 0 && (
            <Typography variant='body2' color='text.secondary' sx={{ mt: 0.5 }}>
              Số dư sau: <strong style={{ color: type === 'add' ? '#0284c7' : '#dc2626' }}>
                {formatVND(currentBalance + (type === 'subtract' ? -Number(amount) : Number(amount)))}
              </strong>
            </Typography>
          )}
        </div>
        ...
        <TextField
          ...
          placeholder='VD: Thưởng affiliate sự kiện, Bù hoa hồng đơn #ORD-123, Điều chỉnh sai sót...'
          required
          multiline
          rows={2}
          ...
          helperText={
            description.length === 0
              ? 'Lý do sẽ hiển thị trong lịch sử giao dịch user'
        ...
        <Typography variant='caption' color='text.secondary' sx={{ display: 'block', mt: 1, fontStyle: 'italic' }}>
          ℹ Cộng vào số dư affiliate (ví hoa hồng). Hiện chưa hỗ trợ rút — user cần đổi sang số dư chính để tiêu.
        </Typography>
      </DialogContent>
      <DialogActions>
