...
**Vấn đề:** Admin con trước chỉ nhập được giá bán cho tier mẹ đã set, không thêm/xóa/sửa tier riêng. Không minh bạch giữa "chính sách mẹ áp với con" và "con đặt giá cho user con".

**Sửa UI ChildServiceFormModal (2 khu):**

**Khu A — Chính sách chiết khấu từ site mẹ (chỉ tham khảo, readonly):**
- Gỡ hết input giá bán, hiển thị thuần tuý
- Đổi tiêu đề + cột: "Giá mẹ bán" (readonly), "Lợi nhuận mẹ gợi ý"
- Giải thích: "Đây là chính sách site mẹ đang áp với bạn"

**Khu B — Chiết khấu riêng của bạn (MỚI):**
- Per mốc thời gian, admin con thêm/sửa/xóa tier: `{min, max?, price}` (max optional = "10+")
- Preview ngay với mỗi tier:
  - Vốn mẹ tại SL min (lookup từ tier mẹ phù hợp)
  - Lãi/proxy
  - Tại SL = min & SL = max: tổng tiền khách trả, tổng vốn, tổng lãi
- Tier lưu vào `metadata.child_quantity_tiers[durationKey]`

**Sửa FE ProxyCard (áp tier con cho user con):**
- `priceOptions` override `quantity_tiers` bằng `metadata.child_quantity_tiers[key]` khi có
- CheckoutModal tự động render card tier con (không cần sửa CheckoutModal)
- `minPricePerUnit` + `maxDiscount` tự xét tier con

**Sửa BE** (xem `../BE/DEVELOPER-GUIDE.md` 15.N+2): `PricingService::getPrice()` fixed mode ưu tiên tier con trước fallback tier mẹ.

**Đồng thời cải tiến UX price_display_unit (13.N+36):** Dropdown → radio + preview giá tính sẵn, áp cho cả Fixed + Per_unit mode (không chỉ per_unit).

**Files:**
- `src/views/Client/Admin/ServiceType/ChildServiceFormModal.tsx`
- `src/views/Client/Admin/ServiceType/ServiceFormModal.tsx`
- `src/app/[lang]/(private)/(client)/components/proxy-card/ProxyCard.tsx`

**Backward compat:** `child_quantity_tiers` null → logic cũ nguyên vẹn. Admin con không set tier riêng = y như trước.

#### 13.N+36 price_display_unit — đơn vị hiển thị giá ProxyCard

**Vấn đề:** Anh Long muốn admin linh hoạt chọn đơn vị hiển thị giá ở trang danh sách, tách khỏi đơn vị bán. VD sản phẩm bán theo ngày 500đ → hiển thị "15.000đ/tháng" (× 30) cho nhìn marketing đẹp hơn.

**Thêm:**
- `ServiceFormModal` (site mẹ): dropdown **"Đơn vị hiển thị giá"** (Mặc định / Ngày / Tháng) cạnh dropdown "Đơn vị bán", trong block per_unit
- `ChildServiceFormModal` (site con): dropdown **"Hiển thị giá theo"** trong header block giá per_unit — site con tự chọn, không ép theo mẹ
- `ProxyCard`: tính `displayUnit = provider.price_display_unit || sellUnit`, convert giá theo công thức `price × (displayFactor / sellFactor)` với day=1, month=30; đổi suffix "/ngày" ↔ "/tháng" theo `displayUnit`

**Công thức convert:**
- sellUnit=day, displayUnit=month: giá × 30 (VD 500 → 15.000)
- sellUnit=month, displayUnit=day: giá / 30 (VD 30.000 → 1.000)
- Cùng đơn vị: giá giữ nguyên

**Không đụng:**
- CheckoutModal (khách chọn mốc bán thật theo đơn vị bán)
- `getDurationLabel` ProxyCard (label mốc thời gian vẫn theo `time_unit` bán, đã fix ở 13.N+35)
- Pricing logic (BE không đổi cách tính tiền)
...
**Backward compat:** Sản phẩm cũ (`price_display_unit = NULL`) → FE fallback về `time_unit` → hiển thị y như trước. `displayRatio = 1`, giá không đổi.

**BE liên quan:** BE cần chạy migration `2026_04_17_000001_add_price_display_unit_to_type_services` + deploy cùng đợt — xem `BE/DEVELOPER-GUIDE.md` section 15.N+1.

#### 13.N+35 Fix label thời gian theo time_unit (sản phẩm bán theo tháng)

**Vấn đề:** Sản phẩm `time_unit=month` vẫn hiển thị label "ngày" / "Tuần" / "Tháng" theo logic hardcode của đơn vị ngày → sai nghĩa (7 tháng hiện "Tuần", "Gia hạn N ngày" khi renew tháng).

**Sửa (scope hẹp — 3 file):**
- `ProxyCard.getDurationLabel`: branch theo `provider.time_unit`. `month` → trả "Tháng"/"Năm"/"N tháng"; `day` giữ nguyên mapping cũ
- `OrderDetail`: renewal label "Gia hạn X ngày" (dòng 1074) + "Số ngày:" (dòng 1124) → dùng biến `unitLabel` đã có sẵn từ `perUnit?.time_unit`
- `CustomPriceModal`: label `${d} ngày` → `${d} ${serviceType?.time_unit === 'month' ? 'tháng' : 'ngày'}`

**Files:**
- `src/app/[lang]/(private)/(client)/components/proxy-card/ProxyCard.tsx`
- `src/views/Client/HistoryOrder/OrderDetail.tsx`
- `src/views/Client/Admin/ServiceType/CustomPriceModal.tsx`

**Backward compat:** `time_unit=day` hoặc null → giữ hành vi cũ, zero regression.

**Chưa fix (sẽ làm đợt khác khi anh báo):** RotatingProxyPage.getDurationLabel (plan đã có time_unit, chỉ cần truyền vào); Admin OrderDetailModal dòng 781/888/1279/1531 + OrderDetail:827 renewal history — cần BE bổ sung `time_unit` trong response `orders` / `order_histories`.
...
#### 13.99 Fix tắt toggle NCC Residential không lưu + không ghi lịch sử

- **Vấn đề**: Tắt toggle "NCC Residential" trong tab Residential của modal Sửa NCC → bấm Lưu → toggle vẫn bật khi mở lại, lịch sử cấu hình không có entry. Nguyên nhân: FE gửi `kind: undefined` khi tắt → `JSON.stringify` drop key undefined → payload không có `kind` → BE `array_merge($existing, $incoming)` giữ nguyên `kind: 'residential'` cũ → DB không đổi → `computeDiff` rỗng → không ghi `ConfigVersion`.
- **Sửa**: `kind: isResidential ? 'residential' : null` — `null` sống sót qua JSON.stringify, BE merge overwrite được. Tất cả consumer BE (8 chỗ) + FE (3 chỗ) đều đọc dạng `(kind ?? null) === 'residential'` → `null` tương đương không có key, backward compatible. `residential_endpoints` + `proxy_host_options` giữ nguyên khi tắt để bật lại không phải nhập lại.
- **Files**: `views/Client/Admin/Provider/ResidentialProviderSection.tsx`

#### 13.100 Nút "Cập nhật" footer modal NCC lưu luôn tab Residential (fix trap 2 nút lưu)

- **Vấn đề**: Modal Sửa NCC có 2 nút lưu: "Lưu cấu hình Residential" (trong tab) và "Cập nhật" (footer). Toggle residential là state cục bộ của section, KHÔNG nằm trong react-hook-form → admin tắt toggle rồi bấm "Cập nhật" footer → `buildApiConfig()` không chứa `kind` → thao tác bị vứt bỏ, không lưu, không có lịch sử. Fix 13.99 chỉ áp dụng cho nút trong tab.
- **Sửa**:
  - Section tách `buildConfig()` (validate dup domain + endpoints required + build 3 key `kind`/`proxy_host_options`/`residential_endpoints`) dùng chung cho cả 2 nút, expose lên parent qua prop `stateRef` (set mỗi render, clear khi unmount)
  - `ModalAddProvider.onSubmit`: nếu `residentialRef.current` tồn tại (= tab Residential đã mở) → gọi `build()` → fail thì toast + nhảy về tab 5, ok thì merge vào `payload.api_config`. Tab chưa mở → ref null → không đụng, BE merge giữ nguyên.
  - Tinh chỉnh: khi tắt toggle mà `kind` vốn không tồn tại → gửi `undefined` (drop) thay vì `null` → không tạo diff lịch sử thừa
- **Files**: `views/Client/Admin/Provider/ResidentialProviderSection.tsx`, `views/Client/Admin/Provider/ModalAddProvider.tsx`

#### 13.101 Redesign + tối ưu perf cấu hình deferred (Mua proxy) — fix select bị che + lag

- **Vấn đề**:
  - **UI**: Nhóm "Kết nối API" (mode deferred) select đầu `sm:2` quá hẹp → label "Cách truyền mã đơn" wrap 2 dòng, giá trị bị cắt bởi padding mũi tên 32px. Hàng tổng >12 cột → field cuối rớt dòng lộn xộn.
  - **Perf**: Modal lag cả 4 thao tác (gõ/đổi select/cuộn/mở). 2 gốc: (1) `BuyConfigSection` render **cả 2 pipeline rotating+static cùng lúc** (ẩn 1 bằng display:none) → mount ~16 Step + hàng trăm field ngay khi mở; (2) mỗi `Step*` gọi 4-7 `useWatch` ở đầu → đổi 1 select re-render toàn subtree ~20 field.
- **Sửa**:
  - **Lazy-render pipeline**: chỉ mount pipeline đã mở (`visitedTypes` Set), pipeline kia chờ user bấm. An toàn data nhờ RHF v7 `shouldUnregister=false` + `reset()` nạp sẵn cả 2 vào store → submit không mất. Giảm ~50% DOM lúc mở.
  - **Tách `DeferredFetchConfig` (memo) + 4 nhóm memo riêng** (`NhomKetNoiApi`/`NhomKiemTraResponse`/`NhomTrichXuat`/`NhomPhanTrang`), mỗi nhóm chỉ `useWatch` field điều khiển của chính nó → đổi select chỉ re-render nhóm liên quan. Gỡ 4 `useWatch` thừa khỏi `StepProxyExtract`.
  - **Redesign 4 nhóm thành card viền màu** (xanh/hổ phách/lục/tím) + icon + grid giãn cân: "Cách truyền mã đơn" `sm:2→3` (hết bị cắt label/value), Params mặc định xuống dòng riêng. (Nhóm Trích xuất ở mode `fields` 6 ô con wrap 4+2 — chấp nhận được, các ô đều cao bằng nhau.) Theo `feedback_config_ui_principles`.
  - Giữ NGUYÊN 100% tên field Controller (`fetch_proxies.*`) → backward compat `buildApiConfig`/`parseApiConfig`.
- **Files**: `views/Client/Admin/Provider/sections/BuyConfigSection.tsx`

#### 13.102 3 ô cấu hình UI cho residential config-driven (Phase 1)

- **Bối cảnh**: BE đưa Proxyma về config-driven deferred (BE 15.46). Cần 3 option mới trong UI để admin cấu hình, nếu không vào UI không thấy chỗ chọn.
- **Sửa**:
  - **Format proxy `auth_string`** (`user:pass@host:port` — residential): thêm `MenuItem` vào dropdown "Format proxy" của bước lấy proxy (`BuyConfigSection` → `NhomTrichXuat`).
  - **Nhãn "Bước dùng"** (`buy`/`fetch`) mỗi custom_field (`ServiceFormModal`): chỉ hiện khi residential. Thêm `stage` vào type `PurchaseOption` + serializer lưu (chỉ lưu khi `fetch`) + load (mặc định `buy`).
  - **"Giá theo số lượng"** (`multiply`/`package`) ở section Chế độ giá (`ServiceFormModal`): chỉ hiện khi residential (`isResidential`). State + UI + save vào `metadata.price_quantity_mode` (chỉ lưu khi `package`) + load + reset. KHÔNG nhầm với `pricing_mode` (fixed/per_unit) sẵn có.
- **Backward compat**: cả 3 đều giá-trị-mới / residential-only / lưu-khi-khác-mặc-định → SP + NCC cũ không đổi. Build compiled OK.
- **Files**: `views/Client/Admin/Provider/sections/BuyConfigSection.tsx`, `views/Client/Admin/ServiceType/ServiceFormModal.tsx`

#### 13.103 Combo location (Phase 2) + hiển thị rotating + total package (Phase 3)

- **Phase 2 — Combo builder + checkout**:
  - `ServiceFormModal`: thêm loại field `combo` (residential-only). Chọn combo → tự khai báo 3 thành phần (country→country_code, region→region_name, city→city) + `stage=fetch`. Builder: bảng các gói, mỗi gói nhập giá trị từng thành phần + cờ + tên → option `{key (tự sinh từ label), label, flag, values}`. Type `PurchaseOption` thêm `components` + option `values`. Save + load đầy đủ.
  - `CheckoutModal`: render `type=combo` thành **1 lưới card** (cờ + tên), click chọn key (bên trong tự bung country/region/city). Type customFields thêm `combo`/`values`/`components`.
- **Phase 3 — Hiển thị + total**:
  - `OrderDetail`: `isResidential=false` cố định → đơn residential render **bảng rotating** (mỗi proxy 1 dòng), bỏ `ResidentialPackageBox` (dead code).
  - `CheckoutModal` + `ProxyCard`: thêm prop `priceQuantityMode`; **total tính theo gói** (`package` → KHÔNG nhân số lượng) → khớp BE. Ô số lượng (`QuantityControl`) tự bật khi `max_quantity>1`.
- **Backward compat**: combo residential-only; `priceQuantityMode` mặc định `multiply` (× SL như cũ). Build compiled OK.
- **Files**: `views/Client/Admin/ServiceType/ServiceFormModal.tsx`, `components/checkout-modal/CheckoutModal.tsx`, `views/Client/HistoryOrder/OrderDetail.tsx`, `app/[lang]/(private)/(client)/components/proxy-card/ProxyCard.tsx`

---

### 07–12/06/2026

#### 13.104 Xoay proxy — 2 mode (thủ công mặc định + tự động khách bật) + IP gốc

- **Admin** `ServiceFormModal` mục "Chế độ xoay IP" (SP xoay): toggle `allow_manual`/`allow_auto` + `min_interval` (chu kỳ tối thiểu, sàn 60s) → `metadata.rotation`. Khai báo field IP gốc NCC ở `RotateSection` (`response.proxy_fields.real_ip`).
- **Khách** `OrderDetail`: mỗi proxy xoay có nút "Xoay / Cài đặt" → mở `ProxyDetailModal`: nút "Lấy proxy/Xoay IP ngay" (gọi `rotate-ip`) + đếm ngược cooldown, toggle "Tự động đổi IP định kỳ" + chọn chu kỳ (gọi `rotation-mode`), hiện "IP gốc (NCC)". Nút màu chủ đạo (brand). Modal mở cả khi đơn mới chưa có proxy. Thêm dòng "IP GỐC" trong bảng order_items.
- API docs (`apiDocsConfig`): sửa response rotate-ip/new/current khớp thực tế + thêm `real_ip`.
- **Files**: `Admin/ServiceType/ServiceFormModal.tsx`, `HistoryOrder/OrderDetail.tsx`, `OrderRotatingProxy/ProxyDetailModal.tsx`, `Admin/Provider/sections/RotateSection.tsx` + `ProviderFormSerializer.ts`/`ProviderFormTypes.ts`, `configs/apiDocsConfig.ts`

#### 13.105 Combo residential — bỏ "Nguồn dữ liệu" + redesign rõ ràng + toggle Card/Dropdown

- Bỏ hẳn picker "Nguồn dữ liệu" (api_tariffs/api_countries) + code chết dependent (api_regions/cities) — net −302 dòng. Field còn 2 băng: Thông tin field → Giá trị khách chọn. Di trú `display_type=country_flag` cho field country picker cũ (chống mất cờ ở checkout khi re-save).
- **Combo** redesign luồng 2 bước: nút "+ Gói vị trí (Combo)"; Bước 1 "Field gửi NCC" (components — thêm/✕ tuỳ ý, `key=param_name`, **bỏ "Mã cột" thừa**) + Bước 2 "Danh sách gói" (mỗi gói: cờ trong input + Value gửi NCC + preview "→ gửi NCC: key=value"). Combo ẩn "Param gửi NCC" cấp field.
- **Toggle hiển thị Card/Dropdown** (`display_type`): nhiều quốc gia → `ComboDropdown` gọn (list cờ+tên cuộn được) trong `CheckoutModal`; mặc định lưới thẻ cờ.
- Fix preview combo: không lọc gói theo `provider_value` (combo dùng flag/values) → preview hiện đúng cờ.
- **Files**: `Admin/ServiceType/ServiceFormModal.tsx`, `components/checkout-modal/CheckoutModal.tsx`

#### 13.106 Nạp tiền — nút mệnh giá hiện đầy đủ số tiền

- `RechargePage` + `RechargeInputDialog`: bỏ viết tắt `num/1000+'k'` → hiện `100.000đ` đầy đủ (khớp admin) — tránh nhầm "100k" thành 100đ.
- **Files**: `views/Client/Recharge/RechargePage.tsx`, `app/.../components/wallet/RechargeInputDialog.tsx`

#### 13.107 Fix hiển thị thông báo trang chủ — thu gọn theo text + toast

- `AnnouncementPost`: `isLong` đo theo **text thật** (strip thẻ HTML) thay vì độ dài HTML thô → không còn thu gọn "Xem thêm" sớm khi nội dung nhiều style/ảnh; bọc `useMemo`.
- Đổi `toast.success` → `toast.info` khi sao chép nội dung (đúng quy tắc không dùng toast.success).
- Đi kèm fix BE 15.50 (tên file ảnh editor unique) — hết lỗi "chọn ảnh mới hiện ảnh cũ" và đè chéo ảnh giữa các thông báo.
- **Files**: `app/[lang]/(private)/(client)/home/page.tsx`

#### 13.108 Quản lý users — thẻ thống kê số admin/user/reseller

- Thêm 3 thẻ thống kê phía trên bảng users: **Số admin** (UserCog, đỏ), **Số user** (UserCheck, xanh), **Số reseller** (Store, teal). Giữ thẻ "Tổng users". Lấy từ BE 15.51 (`admin_count`/`user_count`/`reseller_count`); interface `UserStats` thêm 3 field optional, đọc `?? 0`.
- **Files**: `views/Client/Admin/Users/TableUsers.tsx`, `hooks/apis/useAdminUsers.ts`

#### 13.109 SEO — robots.txt động + dọn sitemap

- **Vấn đề**: `public/robots.txt` tĩnh hardcode `Sitemap: https://mktproxy.com/...` (vỡ multi-site, site con sai domain); `Disallow: /admin/` KHÔNG khớp URL admin thật `/{lang}/admin/...` (có prefix ngôn ngữ) + `Allow: /vi/` còn chủ động cho phép → admin không bị chặn; `Disallow: /private/` vô tác dụng (route group). `sitemap.ts` liệt kê trang 404 (features/support/privacy/terms) và trang private (contact).
- **Sửa**: Xoá `public/robots.txt`, tạo `app/robots.ts` động — `baseUrl=NEXTAUTH_URL` (per-deployment, hết hardcode), chặn `/{locale}/admin/` sinh từ `i18n.locales` + `/admin/` + `/api/`, bỏ `/_next/` (cho Google render) và `/private/`, `Sitemap` theo baseUrl. `sitemap.ts` chỉ còn trang công khai tồn tại: about, cooperate, hotline, pricing, docs.
- Đã verify nội dung prerender thật (`.next/server/app/robots.txt.body`): đủ 5 dòng `/{locale}/admin/` + sitemap absolute.
- **Files**: `app/robots.ts` (mới), `app/sitemap.ts`, xoá `public/robots.txt`

#### 13.110 Quản lý users — ẩn reseller ở site con + thu gọn thống kê 1 hàng

- Ẩn thẻ "Số reseller" khi `isChild` (site con không có reseller) — dùng `useBranding().isChild`.
- Thu nhỏ `StatCard` (icon 34, padding gọn, value subtitle1) + đổi layout Grid2 → flex wrap 1 hàng cho toàn bộ thẻ (site con 6 thẻ gọn 1 hàng desktop). Gỡ import `Grid2` thừa.
- Đi kèm fix BE 15.52 (cột `affiliate_spent`) — trước đó stats lỗi 500 nên mọi thẻ về 0 trên site con.
- **Files**: `views/Client/Admin/Users/TableUsers.tsx`

#### 13.111 Affiliate — thu nhập tháng/tổng + admin cộng tiền affiliate

- **Trang affiliate user**: thay thẻ "Thu nhập tháng này" (đang hiển thị sai = tổng) → dùng `month` đúng; thêm thẻ "Thu nhập tổng" (`total`) + "Số dư affiliate" (`affiliate_balance`, ghi chú *chưa hỗ trợ rút*). Giữ thẻ Hoa hồng %.
- **Admin bảng Users**: nút "Cộng/trừ tiền affiliate" (icon Coins) mỗi user → `ModalAffiliateAdjust` (mirror modal số dư) → `useAdjustAffiliateBalance` → BE 15.53. Cột "Số dư AF" tự cập nhật.
- **Giao dịch**: thêm nhãn `CONG_HOA_HONG_AFFILIATE` = "Cộng hoa hồng (Admin)" ở `UserTransactionModal` + `constants/orderStatus.ts` (dùng chung client + admin).
- **Files**: `app/[lang]/(private)/(client)/affiliate/page.tsx`, `views/Client/Admin/Users/ModalAffiliateAdjust.tsx` (mới), `views/Client/Admin/Users/UsersPage.tsx`, `views/Client/Admin/Users/TableUsers.tsx`, `views/Client/Admin/Users/UserTransactionModal.tsx`, `hooks/apis/useAdminUsers.ts`, `constants/orderStatus.ts`

#### 13.112 Fix SEO title trang chủ không đổi theo cài đặt + thêm cn

- **Vấn đề**: Sửa "Title" ở tab Google & Quảng bá không đổi title hiển thị trên Google. Gốc: `(landing-page)/page.tsx` có `generateMetadata` riêng đè title của layout bằng `site_name - site_description`, **bỏ qua `seo_meta`**. Google hiện title trang chủ → seo_meta.title không bao giờ áp dụng cho trang chủ.
- **Sửa**: `generateMetadata` trang chủ nhận `params`, đọc `seo_meta[lang].title` (fallback `site_name - site_description` → tên → mô tả). description vẫn kế thừa từ layout (không bị đè).
- Thêm ngôn ngữ `cn` (中文) vào tab SEO (`SEO_LANGUAGES` + placeholder) — site có locale `cn` nhưng trước đó không sửa được SEO tiếng Trung. BE validate `seo_meta.*` (wildcard) nên không cần sửa BE.
- **Lưu ý**: title trên site đổi ngay sau lưu+revalidate; Google SERP cập nhật chậm (crawl lag) — dùng Search Console request indexing nếu cần nhanh.
- **Files**: `app/[lang]/(landing-page)/page.tsx`, `views/Client/Admin/SiteSettings/tabs/TabSeo.tsx`

#### 13.113 Fix meta description landing hiện literal {{appName}}

- **Vấn đề**: Description SEO trang Giới thiệu (và cooperate/hotline) hiện literal `{{appName}}` trên Google. Gốc: `generateMetadata` đọc chuỗi dictionary thô từ `locales/{lang}.json` chứa token i18next `{{appName}}`; i18n chỉ thay token khi render component, metadata SSR trả nguyên.
- **Sửa**: helper `interpolateAppName` thay `{{appName}}`/`{{{appName}}}` bằng `getServerBranding().site_name`; áp dụng vào title + description của 3 trang About/Cooperate/Hotline.
- **Files**: `utils/interpolateAppName.ts` (mới), `app/[lang]/(landing-page)/about/page.tsx`, `app/[lang]/(landing-page)/cooperate/page.tsx`, `app/[lang]/(landing-page)/hotline/page.tsx`

#### 13.114 Dashboard — thẻ tổng số dư + tổng số dư affiliate hiện tại

- Thêm hàng 2 thẻ KPI ngay sau ProfitHero: "Tổng số dư người dùng (hiện tại)" (`sum sodu`) + "Tổng số dư affiliate (hiện tại)" (`sum affiliate_balance`). Lấy từ BE 15.54 (`data.balances`).
- Snapshot hiện tại — ghi chú rõ "không phụ thuộc bộ lọc thời gian". `balances` optional + `?? 0` → an toàn khi BE chưa deploy.
- **Files**: `views/Client/Admin/Dashboard/SystemBalancesRow.tsx` (mới), `app/[lang]/(private)/(client)/admin/dashboard/page.tsx`, `hooks/apis/useFinancialReport.ts`
